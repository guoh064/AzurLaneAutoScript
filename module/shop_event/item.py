...
from module.base.utils import save_image, color_similarity_2d
...
class CounterOcr(Ocr):
    ...
    def pre_process(self, image):
        ...
        if len(match):
            left = match[0] + 54  # for jp server
            total = mask.shape[1]
            if left < total:
                image = image[:, left:]
        # save_image(image, f'test/shop_event/dustbin/counter_{time.time()}.png')
    ...
    def pre_process(self, image):
        ...
        if len(match):
            ...
            if left < total:
                image = image[:, left:]
        image = super().pre_process(image)
        # save_image(image, f'test/shop_event/dustbin/price_{time.time()}.png')
        return image
...
class EventShopItem(Item):
    ...
    def predict_valid(self):
        ...
        if mean >= 0.3:
            return True
        # save_image(mask, f'test/shop_event/dustbin/valid_{time.time()}.png')

        result = TEMPLATE_UNOBTAINED.match(mask.astype(np.uint8), similarity=0.75)
        if result:
            self.tag = 'unobtained'
        return result
