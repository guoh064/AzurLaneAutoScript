...
from module.base.utils import color_similarity_2d, crop, save_image
...
from module.shop_event.item import ITEM_SHAPE, EventShopItemGrid, DELTA_PRICE_BACKGROUND, DELTA_ITEM, DELTA_PRICE, \
...
class EventShopClerk(EventShopUI):
    ...
    def event_shop_buy_item(self, item_to_buy, amount=None):
        ...
        items = [item for item in items if str(item) == str(item_to_buy)]
        if len(items) == 0:
